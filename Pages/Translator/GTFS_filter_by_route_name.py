...
def run():
	...
	def main():
		...
		if input_file is not None:
			...
			routes_to_filter = st.multiselect("Select route_short_name (within routes.txt) to filter",
											  routes_df['route_short_name'].unique().tolist())
	...
	def gtfs_filtering(zip_file, df_routes):
		...
		routes=read_csv(zip_file.open('routes.txt'), header=0, dtype=str).join(df_routes, on='route_short_name', how='inner').reset_index(drop=True)
		if 'trips.txt' not in input_file_list:
			exit('\nThere is no trips file in this folder. Aborting.\n')
		logger.write('Reading trips file')
		df['trips'] = read_csv(zip_file.open('trips.txt'), header=0, dtype=str).merge(routes, on='route_id', how='inner').reset_index(drop=True)
		df['trips'].drop(['agency_id','route_short_name','route_long_name','route_type'], axis=1)
		trips = read_csv(zip_file.open('trips.txt'), header=0, dtype=str).merge(routes, on='route_id', how='inner').reset_index(drop=True)
		trips.drop(['agency_id','route_short_name','route_long_name','route_type'], axis=1)
