...
class Instructions:
    instructions = 'Upload the Hastus File and run the script to download a Schedule file'
    link = 'https://optibus.atlassian.net/wiki/spaces/OP/pages/3235151981/Hastus+Into+Data+set+Schedule+Scripts#Hastus-Sweden-Into-Vehicle-Schedule-File'

class Description:
    title = "Hastus Sweden Into Schedule"
    description = "This is a script that enables you to convert Hastus format which common in sweden into Schedule file, see confluence page for more instructions"
...
def run():
    ...
    def main():
        ...
        if input_file!=None:
            ...
            excel_data=write_excel(trips)
            st.download_button('Download File', data=excel_data, file_name='output.xlsx',
    ...
    def creating_trips_file(zip_file):
        ...
        df_2 = pd.read_csv(zip_file.open('expohastFull.txt'), header=None, dtype=str, encoding='unicode_escape')
        ...
        for i in range(df_2.shape[0]):
            # Check if the current row is a "trip" row
            if df_2.iloc[i, 0] == "block":
                # If it is, store the values from columns 3 and 4 in the lists
                trip_col3.append(df_2.iloc[i, 1])
            # Check if the current row is a "tp" row
            elif df_2.iloc[i, 0] == "trip":
                # If it is, append the values from the lists to columns 3 and 4
                df_2.iloc[i, -1] = trip_col3[-1]
        # Create two new DataFrames, one for the "trip" rows and one for the "tp" rows
        trip_df_2 = df_2.loc[df[0] == "trip"]
        trip_df_2=trip_df_2[[2,3,15]]
        trip_df_2.columns=['Sign','trip_id','Vehicle Block Id']
        trip_df_2['Id'] = trip_df_2[["Sign", 'trip_id']].apply(
            lambda x: '_'.join(x.dropna()), axis=1)
        trip_df_2=trip_df_2[['Id','Vehicle Block Id']]
        Trips = pd.merge(Trips, trip_df_2, on='Id')
        Trips['Event Type']='service_trip'
        Trips['Description']=''
        Trips['Origin Stop Name'] = ''
        Trips['Destination Stop Name'] = ''
        Trips['Required Vehicle Type'] = ''
        Trips['First Stop Name'] = ''
        Trips['Last Stop Name'] = ''
        Trips['Overlap'] = ''
        Trips['Pref Group'] = ''
        Trips['Route Id']
        Trips=Trips[['Vehicle Block Id','Event Type','Description','Id','Route Id','Sign',"Direction",'Alternative','Day Offset',"Departure", "Arrival",'Boarding Time',"Origin Stop Id",'Origin Stop Name','Destination Stop Id','Destination Stop Name','Required Vehicle Type',"Vehicle Type Ids",'Distance','First Stop Name','Last Stop Name','Overlap','Days','Pref Group']]
        Trips.columns=['Vehicle Block Id','Event Type','Description','Trip Id','Route Id','Sign',"Direction",'Alternative','Day Offset',"Start Time", "End Time",'Boarding Time',"Origin Stop Id",'Origin Stop Name','Destination Stop Id','Destination Stop Name','Required Vehicle Type',"Actual Vehicle Type",'Distance','First Stop Name','Last Stop Name','Overlap','Days','Pref Group']
        return Trips
    ...
    def write_excel(table_array):
        ...
        table_array.to_excel(excel, sheet_name='Sheet 1', merge_cells=False, freeze_panes=[1, 0], index=False)
        excel.close()
        output.seek(0)
