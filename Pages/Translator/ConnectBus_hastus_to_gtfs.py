...
def map_stops_to_timepoints(df_all, df_stops):
    ...
    df_stops['ID'] = df_stops['ID'].astype(str)
    stop_dict = df_stops.set_index('ID')[['LATITUDE', 'LONGITUDE','STOP NAME']].to_dict('index')
    key_type = type(list(stop_dict.keys())[0]) if stop_dict else None
    print('Data type of first key in stop_dict:', key_type)
    print('"2002" exists in stop_dict:', "2002" in stop_dict)
    print('Value for "2002" in stop_dict:', stop_dict.get("2002"))
    ...
    for i, row in df_all.iterrows():
        ...
        for timepoint in row['timepoints']:
            ...
            if timepoint['stop_id'] in stop_dict:
                ...
                pass
                # Print the debug information if stop_id is not found
                #print(f"Stop ID {timepoint['stop_id']} not found in stop_dict.")
                #print(f"Timepoint format: {timepoint}")
...
def create_gtfs(df_all):
    ...
    def create_stop_times(row):
        distances = [float(tp['distance']) for tp in row["timepoints"]]
        cumulative_distances = [round(value, 3) for value in list(accumulate(distances))]

        stop_times_list = [{
            "trip_id": row['trip_id'],
            "arrival_time": tp["time"] if len(tp["time"].split(":")) == 3 else tp["time"] + ":00",
            "departure_time": tp["time"] if len(tp["time"].split(":")) == 3 else tp["time"] + ":00",
            "stop_id": tp["stop_id"],
            "stop_sequence": idx,
            "shape_dist_traveled": cumulative_distances[idx],
            "timepoint": int(tp["timepoint"])  # Convert boolean to int
        } for idx, tp in enumerate(row["timepoints"])]

        return pd.DataFrame(stop_times_list)
...
def app():
    ...
    def perform_processing():
        ...
        if zip_file and pdf_file and stop_file and uploaded_file:
            ...
            st.write(df_all.iloc[2].to_json())
            ...
            st.write(f'number of trips with missing stop point references in sequence{len(reference_df)}')
            ...
            st.write(unique_unmapped_stop_ids)
...
#the next steo is to check the vehicle types for the schedule, and next this





# Identifier - just use the /x, split for this identifier (don't need direction) - use the file name. 

#also use direction in new column called direction this is after the comma in the filename

#concatenate with semi colon to TripNumber. 

#;identifier;TripNumber;

# map this to the expohastfull trip

#Also use this as new column called Trip_No (match what the ds requirement) - identifier_tripNumber (used for the trip_id)



# trip;0;113;915;10;0;0;1;0;0;0;0;4.614;NF;rK_113;

# first 0 unsure
#113 identifier 
# 915 is Trip Number
#Unsure what 10 is 
# next 7 ints seperated by semicolon are days of week starting on Monday
# total trip distance (4.164)
# NF is vehicle type 
# rK_113 - reference to a note in a different file. 


# This data is below the corresponding trip 

# tp stands for timepoint 
# 1 is tp and 0 is not
# 5 digit is the stop id 
# 1410 is the time on stop 
# distance is the last value between n and n-1 
# Sometimes there will be an addition string after the semicolon on the same line for the timepoint, just remove it



# Use the period and insper, in the service id: as an example 14!3 which is period 14 and insPer 3, and also use the days as part of this identifier and then we can group them. 

#Service id construction

# 14!3;1-5 
# See if we can get the hyphen back for the days in the reader as important. 

# Map long lat values that are exact mtach to this dataframe - uploader is excel file. 