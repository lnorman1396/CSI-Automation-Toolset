...
def run():
    ...
    def validate_VehicleTypes(excel_sheet):
        ...
        if special_chars.any():
            errors.append((id_col, 'Found special characters in column 0'))
    ...
    def validate_Places(excel_sheett):
        ...
        column_mappings = {
            'id': ["Stop Id", "Id", "קוד"],
            'description': ["Stop Name", "name", "Description", "שם"],
            ...
            if key == 'id':
                ...
                if special_chars.any():
                    ...
                    for index, row in special_chars.iterrows():
                        errors.append((df_columns['id'], 'Special character found', index+2))
        ...
        # Check if lat >= lon
        if (lat < lon).any():
            wrong_values = excel_sheet[lat < lon]
            for index, row in wrong_values.iterrows():
                errors.append((df_columns['latitude'], 'Latitude is not greater than or equal to Longitude', index+2))
    ...
    def validate_trips(dff):
        ...
        expected_column_values = {
            4: ["0","1","2", "Inbound","Outbound","Circular", 0, 1, 2],
            8: ["Yes", "No","1", "0", 1, 0]
        }
        ...
        for column_index, expected_names in expected_column_names.items():
            ...
            if column_index == 0:  # Check for special characters
                if df[column_name].str.contains('[^\w\s]').any():
                    ...
                    for row in error_rows:
                        error_df.loc[next_index] = [column_index, row+2, "Special characters are not allowed."]
                        next_index += 1
                    ...
                    if not (pd.isna(row[df.columns[2]]) and pd.isna(row[df.columns[3]])):
                        if row[df.columns[2]] != row[df.columns[3]]:
                            error_df.loc[next_index] = [column_name, index+2,
                                                        "When the 'Alternative' column is not empty, sign column and catalog column should have the same values and not empty"]
                            next_index += 1
    ...
    def validate_trips_with_stoptimes(excel_sheet):
        ...
        id_check_mask = trips['Id'].isin(stoptimes['Trip Id'])
        ...
        merged_df = pd.merge(trips, stoptimes, how='inner', left_on='Id', right_on='Trip Id')

        # Check if "Sequence" or "sequence" exists and use the one that exists
        ...
        if not all(first_sequence_df['Departure'] == first_sequence_df['Time']):
            temp_df = first_sequence_df.loc[first_sequence_df['Departure'] != first_sequence_df['Time'], ['Id']].copy()
        ...
        if not all(last_sequence_df['Arrival'] == last_sequence_df['Time']):
            temp_df = last_sequence_df.loc[last_sequence_df['Arrival'] != last_sequence_df['Time'], ['Id']].copy()
        ...
        if not all(first_sequence_df['Origin Stop id'] == first_sequence_df['Point Id']):
            temp_df = first_sequence_df.loc[first_sequence_df['Origin Stop id'] != first_sequence_df['Point Id'], ['Id']].copy()
            temp_df['Error'] = "Origin Stop id does not match the Point Id at first sequence in stoptimes."
            error_df = pd.concat([error_df, temp_df], ignore_index=True)

        # Check if Destination Stop Id in trips matches Point Id at last sequence in stoptimes
        if not all(last_sequence_df['Destination Stop Id'] == last_sequence_df['Point Id']):
            temp_df = last_sequence_df.loc[last_sequence_df['Destination Stop Id'] != last_sequence_df['Point Id'], ['Id']].copy()
    ...
    def validate_trips_with_veihcles(excel_sheet):
        ...
        if 'VehicleTypes' not in wb.sheetnames:
            ...
            return df
        Trips = pd.read_excel(excel_sheet, sheet_name='Trips')
        VehicleTypes = pd.read_excel(excel_sheet, sheet_name='VehicleTypes')
